...
func TestAuthorizationHandler_Authorize(t *testing.T) {
	...
	t.Run("Successful authorization", func(t *testing.T) {
		mockSessionUseCase.On("GetSession", mock.Anything, mock.Anything).Return(0, nil) // Сессия не была установлена, значит пользователь не авторизован
...
func TestAuthorizationHandler_Authorize_UserNotFound(t *testing.T) {
	...
	_, err = handler.Authorize(ctx, user)
	assert.Error(t, err)
