...
func (us *UserStorage) SaveUser(email, password, salt string) error {
	...
	_, err := us.db.Exec(us.ctx, `INSERT INTO "user"(email, passwrd, salt) VALUES ($1, $2, $3)`, email, password, salt)
	return err
}
...
func (us *UserStorage) ChangeEmail(userID int, email string) error {
	...
	defer us.mu.Unlock()
	us.ctx = context.Background()
	_, err := us.db.Exec(us.ctx, `UPDATE "user" SET email = $1 WHERE id = $2`, email, userID)
	return err
}
...
func (us *UserStorage) ChangePassword(userID int, password, salt string) error {
	...
	defer us.mu.Unlock()
	us.ctx = context.Background()
	_, err := us.db.Exec(us.ctx, `UPDATE "user" SET passwrd = $1, salt = $2 WHERE id = $3`, password, salt, userID)
	return err
}
...
func (us *UserStorage) GetUserByID(id int) (entities.User, error) {
	...
	err := us.db.QueryRow(us.ctx, `SELECT id, email, passwrd, salt FROM "user" WHERE id = $1`, id).Scan(&user.ID, &user.Username, &user.Password, &user.Salt)
	if err != nil {
...
func (us *UserStorage) GetUserByEmail(email string) (entities.User, error) {
	...
	us.ctx = context.Background()
	var user entities.User
	err := us.db.QueryRow(us.ctx, `SELECT id, email, passwrd, salt FROM "user" WHERE email = $1`, email).Scan(&user.ID, &user.Username, &user.Password, &user.Salt)
	if err != nil {
		return entities.User{}, err
	}
...
func (us *UserStorage) DeleteUser(userID int) error {
	...
	defer us.mu.Unlock()
	us.ctx = context.Background()
	_, err := us.db.Exec(us.ctx, `DELETE FROM "user" WHERE id = $1`, userID)
	return err
}
...
func (us *UserStorage) IsEmailTaken(email string) (bool, error) {
	...
	var count int
	err := us.db.QueryRow(us.ctx, `SELECT COUNT(*) FROM "user" WHERE email = $1`, email).Scan(&count)
	if err != nil {
		return false, err
	}
	return count > 0, nil
